...
impl Dice {
    ...
    pub(crate) fn start(&self) -> Option<Bounded> {
        if let Some(crit) = self.critical() {
            let start = *self.faces().start();
            Some(Bounded::from_range(..(start + crit)))
        } else {
            None
        }
    }

    pub(crate) fn end(&self) -> Option<Bounded> {
        if let Some(crit) = self.critical() {
            let end = *self.faces().end();
            Some(Bounded::from_range(end + 1 - crit..))
        } else {
            None
        }
    }
...
mod test {
    ...
    fn check_d2_values() {
        ...
        assert_eq!(Dice::D2.start(), Some(Bounded::from_range(..2)));
        assert_eq!(Dice::D2.end(), Some(Bounded::from_range(2..)));
    }
